...
//returns hash for the string s
int hash(string s, int table_size)
{
  unsigned int i, h = 0;
  for (i=0; i<s.length(); i++)
    h = (h * 2002 + (unsigned int)s[i]) % table_size;
  return h;
}

// Allocate a table of pointers to Page struct
...
webPage::webPage(int tablesize)  //constructor to call allocate table function
{
  size=tablesize;
...
webPage::~webPage()    // destructor to delete table size
{
  for (int i=0; i<size; i++)
  {
    while(table[i] != NULL)
...
//checks the hash table if it is full ....if so then i creates another table and rehashing of all elements takes place
